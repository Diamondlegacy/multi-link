...
export default async function handler(req, res) {
  ...
  // All-time total hours logged by workers (admins don't count toward payroll)
  ...
  // This week's hours (Monday through Sunday, server's clock)
  const weekResult = await query(`
    SELECT COALESCE(SUM(h.hours), 0) AS total_hours
    FROM hours_entries h
    JOIN users u ON u.id = h.user_id
    WHERE u.role = 'worker'
      AND h.entry_date >= date_trunc('week', CURRENT_DATE)
      AND h.entry_date < date_trunc('week', CURRENT_DATE) + interval '7 days'
  `);
  ...
  return res.status(200).json({
    ...
    thisWeek: { hours: weekHours, earnings: weekEarnings },
    adminCutThisWeek,
